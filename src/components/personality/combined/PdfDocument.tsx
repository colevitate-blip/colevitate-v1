...
export const PdfDocument = forwardRef<
  HTMLDivElement,
  { profile: CombinedProfileData; results: PersonalityResults; careerSuggestions: string[]; locale?: string }
>(function PdfDocument({ profile, results, careerSuggestions, locale }, ref) {
  const t = useTranslations("combined.ui.pdf");
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <div style={{ fontSize: 18, fontWeight: 700, letterSpacing: -0.2 }}>{tChrome("brand")}</div>
          <div style={{ fontSize: 12, color: INK_FAINT, marginTop: 2 }}>{tChrome("tagline")}</div>
        </div>
      ...
      <Section>
        <SectionTitle>{t("whereLensesAgree")}</SectionTitle>
      ...
      {/* Where to lean in */}
      <Section>
        <SectionTitle>{t("whereToLeanIn")}</SectionTitle>
      ...
      {careerSuggestions.length > 0 ? (
        <Section>
          <SectionTitle>{t("whereThisMightFit")}</SectionTitle>
      ...
      <Section>
        <div style={{ display: "flex", gap: 32 }}>
          ...
          <div style={{ flex: 1 }}>
            <SectionTitle>{t("growthAreas")}</SectionTitle>
            <ul style={{ margin: 0, paddingLeft: 18, fontSize: 13, lineHeight: 1.65, color: INK }}>
      ...
      {/* Your four lenses */}
      <Section>
        <SectionTitle>{t("yourFourLenses")}</SectionTitle>
