...
export function DailyTypeInsight({
  archetypeKey,
  archetypeName,
  locale,
}: {
  ...
  const t = useTranslations("combined.ui.dailyInsight");
  const [content, setContent] = useState<string | null>(null);

  useEffect(() => {
    let cancelled = false;
    fetch(`/api/daily-type-content?key=${encodeURIComponent(archetypeKey)}&locale=${encodeURIComponent(locale)}`)
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <div className="mt-4 flex items-start gap-2.5 rounded-2xl border bg-muted/20 p-4">
      ...
      <div>
        <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("todayFor", { name: archetypeName })}</p>
