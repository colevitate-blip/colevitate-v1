...
/**
 * /tm1 — real landing page, real header/footer/data, with ONLY the hero
 * section replaced by a condensed alternative (everything below the hero is
 * byte-for-byte the same as the live homepage). This is Model 1 ("compact
 * stack"): same centered-badge/headline/subtitle shape as today, just turned
 * down — smaller pill, tighter headline scale, subtitle capped to 2 lines on
 * mobile — so it's a real comparison, not an abstract mockup.
 */
export default function Tm1Page() {
  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-4 pb-24 pt-6 sm:pt-10">
      {/* --- Model 1 hero (only change vs. the live page) --- */}
      <section className="mx-auto max-w-2xl text-center">
        <span className="mb-2.5 inline-flex items-center gap-1 rounded-full border px-2.5 py-0.5 text-[10px] font-medium uppercase tracking-wide text-primary">
        ...
        <p className="mx-auto mt-2 line-clamp-2 max-w-md text-sm leading-relaxed text-muted-foreground sm:mt-3 sm:line-clamp-none sm:max-w-none sm:text-base">
          {t("subtitle")}
        </p>
      </section>
      {/* --- end Model 1 hero --- */}

      <section className="mt-8 grid gap-5 sm:mt-14 sm:grid-cols-2">
