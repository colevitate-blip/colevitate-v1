#!/usr/bin/env bash
# ==============================================================================
# qa-setup.sh — Tiered QA scaffolding for this Next.js + npm + Vercel site
#
# Adapted from qa-setup.md (a generic Astro + pnpm template) for this repo:
#   - default QA_BASE_URL is localhost:3000 (Next.js dev), not Astro's 4321
#   - package-manager detection prefers npm (package-lock.json here)
#   - qa:all and the CI workflow run via `npm run`, not hardcoded `pnpm`
#   - Tier 8's CI workflow builds+starts with `next build`/`next start`
#     (there's no Astro-style `preview` script) and passes the Supabase
#     public env vars a build needs (see .env.local)
#
...
#   QA_BASE_URL=http://localhost:3000 ./qa-setup.sh
# ==============================================================================

set -euo pipefail

ALL_TIERS=(0 1 2 3 4 5 6 7 8)
QA_BASE_URL_DEFAULT="http://localhost:3000"
...
tier0_setup() {
  ...
  if [[ "$mgr" == "pnpm" ]]; then
    ...
    add_pkg_script "qa:all" "npm run qa:links && npm run qa:console && npm run qa:mobile && npm run qa:lighthouse"
  fi
...
FILEEOF
  ...
  echo "  (needs NEXT_PUBLIC_SUPABASE_URL / NEXT_PUBLIC_SUPABASE_ANON_KEY set as repo secrets)"
}
...
echo
echo "Done. Start the site (\`npm run dev\` or \`npm run build && npm run start\`), then e.g.:"
echo "  npm run qa:links && npm run qa:console && npm run qa:mobile"
